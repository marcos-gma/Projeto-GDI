...
CREATE OR REPLACE TYPE TP_ENDERECO AS OBJECT (
    ...
    CONSTRUCTOR FUNCTION TP_ENDERECO(SELF IN OUT TP_ENDERECO, CEP VARCHAR2, RUA VARCHAR2, NUMERO NUMBER, BAIRRO VARCHAR2) RETURN SELF AS RESULT
);
/
...
CREATE OR REPLACE TYPE TP_SENTENCA AS OBJECT (
    ...
    CONSTRUCTOR FUNCTION TP_SENTENCA(SELF IN OUT NOCOPY TP_SENTENCA, CRIME VARCHAR2, DETENTO TP_DETENTO, DURACAO NUMBER) RETURN SELF AS RESULT
);
/

CREATE OR REPLACE TYPE BODY TP_SENTENCA AS
    CONSTRUCTOR FUNCTION TP_SENTENCA(SELF IN OUT TP_SENTENCA, CRIME VARCHAR2, DETENTO TP_DETENTO, DURACAO NUMBER) RETURN SELF AS RESULT IS
    BEGIN
        SELF.CRIME := CRIME; SELF.DETENTO := DETENTO; SELF.DURACAO = DURACAO;
        RETURN SELF;
    END;
END;
/